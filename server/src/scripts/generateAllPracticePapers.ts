...
async function generateAllPracticePapers() {
    console.log("Generating practice papers for all chapters...\n");
    const db = await initDatabase();
    
    // Get all chapters
    const chapters = await db.all(`
        SELECT c.id, s.id as subject_id, s.name as subject, c.name as chapter,
    ...
    for (const chapter of chapters) {
        ...
        if (!paper) {
            console.log(`⚠️  No practice paper defined for: ${key}`);
            continue;
        }

        console.log(`✅ Inserting practice paper for: ${key}`);
    ...
    console.log(`\n✨ Done!`);
    console.log(`   Inserted: ${inserted} practice papers`);
    console.log(`   Skipped: ${skipped} chapters (already have papers)`);
...
generateAllPracticePapers().catch(console.error);
